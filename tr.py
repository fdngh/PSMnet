...
from models.DVD1 import FewShotSeg
...
def main(_run, _config, _log):
    ...
    log_loss = {'loss': 0, 'align_loss': 0, "query_loss1": 0}
        ...
        for _, sample_batched in enumerate(trainloader):
            ...
            try:
                query_pred, align_loss = model(support_images, support_fg_mask, support_bg_mask, query_images,
                                               tra_wsize=_config["tra_wsize"], isval=False, val_wsize=None)

            except ZeroDivisionError as e:
                continue

            query_loss = criterion(query_pred, query_labels)
            query_loss1 = dice_loss(query_pred, query_labels, eps=1e-7)
            
         

            loss = query_loss + 0*align_loss + 0*query_loss1
            loss.backward()
            optimizer.step()
            scheduler.step()
            ...
            query_loss1 = query_loss1.detach().data.cpu().numpy()
            _run.log_scalar('loss', query_loss)
            _run.log_scalar('align_loss', align_loss)
            _run.log_scalar('zhi', query_loss1)
            log_loss['loss'] += query_loss
            log_loss['align_loss'] += align_loss
            log_loss['query_loss1'] += query_loss1

            # print loss and take snapshots
            if (i_iter + 1) % _config['print_interval'] == 0:
                loss = log_loss['loss'] / _config['print_interval']
                align_loss = log_loss['align_loss'] / _config['print_interval']
                zhi_loss = log_loss['query_loss1'] / _config['print_interval']

                log_loss['loss'] = 0
                log_loss['align_loss'] = 0
                log_loss['query_loss1'] = 0

                print(f'step {i_iter + 1}: loss: {loss}, align_loss: {align_loss},query_loss1:{zhi_loss}')
